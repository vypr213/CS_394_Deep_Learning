...
try:
    ...
    print("Package termcolor not found. Disabling colored output.")
    colored = None
...
class Grader:
    ...
    def run(self, logger):
        ...
        for _, f, _ in self.get_all_cases():
            ...
            if self.verbose:
                log = logger.info if abs(s - f.score) < 1e-2 else logger.warn
                log(f"  - {f.__doc__.strip():<50} [ {s} / {f.score} ]")
...
class RuntimeFormatter(logging.Formatter):
    ...
    def __init__(self, *args, disable_color: bool = False, **kwargs):
        ...
        self.disable_color = disable_color or colored is None
